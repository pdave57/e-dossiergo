...
func (s *ZonalSummaryService) GetZoneSummaryReport(ctx context.Context, schoolID string) ([]domain.ZoneSummaryReport, error) {
	session, err := s.sessionRepo.GetActive(ctx, schoolID)
	if err != nil {
		return nil, err
	}

	return s.zonalReportRepo.GetZoneSummaryReport(ctx, session.ID)
}
